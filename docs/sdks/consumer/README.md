...
async function run() {
  const result = await sdk.consumer.createConsumer({
...
import { consumerCreateConsumer } from "openapi/funcs/consumerCreateConsumer.js";
...
async function run() {
  const res = await consumerCreateConsumer(sdk, {
    projectIdOrName: "<value>",
    requestBody: {
      consumerRef: "consumer-123",
      consumerName: "John doe",
