...
```bash
npm add https://github.com/amp-labs/amp-sdk-ts
```

### PNPM

```bash
pnpm add https://github.com/amp-labs/amp-sdk-ts
```

### Bun

```bash
bun add https://github.com/amp-labs/amp-sdk-ts
```

### Yarn

```bash
yarn add https://github.com/amp-labs/amp-sdk-ts zod
...
async function run() {
  let result;
  try {
    result = await sdk.write.records({
      projectIdOrName: "<value>",
      integrationId: "<id>",
      objectName: "<value>",
      requestBody: {
    ...
    switch (true) {
      ...
      case (err instanceof WriteRecordsResponseBody): {
        // Handle err.data$: WriteRecordsResponseBodyData
        console.error(err);
        return;
      }
      default: {
